...
namespace {
    ...
    struct ImageInfo {
        ...
        struct Times {
            ...
            std::optional<double> gps_seconds;
        } times;
    ...
    ImageInfo::CameraSettings get_exif_camera_settings(const std::optional<Rational<uint32_t>>& f_number,
            ...
            const std::optional<uint16_t>& focal_length_35mm,
        ...
        if (focal_length_35mm && focal_length_35mm != 0) {
            settings.focal_length = format_string("%d mm (35mm film equivalent)", *focal_length_35mm);
        } else if (const auto rational = normalize_rational(focal_length)) {
            if (rational->numerator != 0) {
                settings.focal_length = format_string("%d mm", std::lround(rational_to_double(*rational)));
            }
        }
    ...
    std::pair<ImageInfo::GPS, std::optional<double>> get_exif_gps(
        ...
        std::optional<double> gps_date_time{};
        if (gps_date_stamp) {
            ...
            if (date::from_stream(date_time_str_stream, "%Y:%m:%d", gps_date)) {
                ...
                gps_date_time = gps_date.time_since_epoch().count();
            }
        }
    ...
    std::optional<ImageInfo> load_raw_image_info(const char* path) {
        ...
        std::tie(info.gps, info.times.gps_seconds) = get_exif_gps(exif.gps_latitude_ref, exif.gps_latitude, exif.gps_longitude_ref,
    ...
    std::optional<std::string_view> try_get_string_field(const vips::VImage& image, const char* name) {
        ...
        // Format: "<value> (<pretty value>, ASCII, N components, B bytes)",
        static constexpr std::string_view tag_info_separator{", "};
        const auto byte_count_pos = string.rfind(tag_info_separator);
        if (byte_count_pos == std::string::npos) {
            return std::nullopt;
        }

        const auto byte_count_str = string.substr(byte_count_pos + tag_info_separator.size());
        size_t byte_count{};
        if (std::from_chars(byte_count_str.begin(), byte_count_str.end(), byte_count).ec != std::errc{}) {
            return std::nullopt;
        }

        // Should have at lest one byte for the null terminator
        if (byte_count <= 0) {
            return std::nullopt;
        }

        return string.substr(0, byte_count - 1);
    }
    ...
    std::optional<ImageInfo> load_vips_image_info(const char* path) {
        ...
        ImageInfo info{.width = image.width(), .height = image.height()};
        ...
        std::tie(info.gps, info.times.gps_seconds) = get_exif_gps(
...
PYBIND11_MODULE(cru, m) {
    ...
    py::class_<ImageInfo::Times>(m, "Times").
        ...
        def_readonly("gps_seconds", &ImageInfo::Times::gps_seconds);
