...
def try_load_image_info(path: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        if data.times.gps_seconds:
            fields['gpsDateTime'] = datetime.fromtimestamp(data.times.gps_seconds, timezone.utc).isoformat()
...
def status():
    def get_event_data(wait: bool = True):
        with display_writer_update_lock:
            ...
            if display_writer_image_info:
                data['imageInfo'] = display_writer_image_info
